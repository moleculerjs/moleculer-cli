		...
		Promise.resolve()
					...
					fs.exists(configPath, (configExists) => {
						if (configExists) {
							fs.readFile(configPath, (err, config) => {
								if (err) {
									return reject(
										`Error reading config file from ${configPath}`
									);
								}
				...
				if (values.templateRepo) {
					return new Promise((resolve, reject) => {
						...
						download(values.templateRepo, values.tmp, {}, (err) => {
							if (err)
								return reject(
									`Failed to download repo from '${values.templateRepo}'!`,
									err
								);
						...
						.prompt([
							{
								...
								message: kleur
									.yellow()
									.bold(
										`The '${values.projectName} directory is exists! Continue?`
									),
								default: false,
							},
						])
			...
			.then(() => {
				return new Promise((resolve, reject) => {
					...
					if (templateMeta.helpers)
						Object.keys(templateMeta.helpers).map((key) =>
							Handlebars.registerHelper(
								key,
								templateMeta.helpers[key]
							)
						);

					// metalsmith.before
					if (
						templateMeta.metalsmith &&
						_.isFunction(templateMeta.metalsmith.before)
					)
						templateMeta.metalsmith.before.call(
							templateMeta,
							metalsmith
						);
					...
					metalsmith
						.use(
							filterFiles(templateMeta.filters, templateMeta.skip)
						)
					...
					if (
						templateMeta.metalsmith &&
						_.isFunction(templateMeta.metalsmith.after)
					)
						templateMeta.metalsmith.after.call(
							templateMeta,
							metalsmith
						);
					...
					metalsmith
						...
						.build((err) => {
							...
							if (
								templateMeta.metalsmith &&
								_.isFunction(templateMeta.metalsmith.complete)
							)
								templateMeta.metalsmith.complete.call(
									templateMeta,
									metalsmith
								);
